...
export default function ContactPage() {
  ...
  return (
    <>
      {/* Hero */}
      <section className="pt-24 md:pt-32 pb-10 md:pb-16 bg-[#0F1F4B] text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <span className="inline-block px-4 py-2 bg-white/10 rounded-full text-sm font-bold mb-5">
              {tr.contact.badge}
            </span>
            <h1
            ...
            <p className="text-white/70 text-base sm:text-lg max-w-xl">{tr.contact.subtitle}</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-10 md:py-24 bg-[#F8F7F4]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12">
            ...
            {/* Info — shows below form on mobile */}
            <AnimatedSection direction="right" className="lg:col-span-2 order-2 lg:order-1">
              <div className="grid grid-cols-2 lg:grid-cols-1 gap-3 mb-5">
                {infoItems.map(({ icon: Icon, label, value }, i) => (
                  <div key={i} className="flex items-center gap-3 bg-white rounded-2xl p-4 shadow-[0_2px_12px_rgba(15,31,75,0.08)]">
                    <div className="w-9 h-9 md:w-11 md:h-11 bg-[#0F1F4B] rounded-xl flex items-center justify-center flex-shrink-0">
                      <Icon size={16} className="text-white" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs text-gray-400 font-semibold uppercase tracking-wider leading-none mb-0.5">{label}</p>
                      <p className="font-semibold text-[#0F1F4B] text-sm truncate">{value}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {/* Map */}
              <div className="bg-[#0F1F4B] rounded-2xl overflow-hidden h-40 md:h-48 relative">
            ...
            <AnimatedSection direction="left" className="lg:col-span-3 order-1 lg:order-2">
              <div className="bg-white rounded-3xl p-5 sm:p-8 shadow-[0_4px_32px_rgba(15,31,75,0.10)]">
                <h2
                  className="text-xl sm:text-2xl font-black text-[#0F1F4B] mb-5"
                  style={{ fontFamily: "Montserrat, sans-serif" }}
                >
                  {f.send}
                </h2>
                ...
                {status === "success" && (
                  <div className="flex items-center gap-3 bg-green-50 text-green-700 rounded-xl p-4 mb-5">
                    ...
                    <span className="font-medium text-sm">{f.success}</span>
                  </div>
                )}
                {status === "error" && (
                  <div className="flex items-center gap-3 bg-red-50 text-red-600 rounded-xl p-4 mb-5">
                    <AlertCircle size={18} className="shrink-0" />
                    <span className="font-medium text-sm">{f.error}</span>
                  </div>
                )}
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-semibold text-[#0F1F4B] mb-1.5">{f.name}</label>
                      <input
                        {...register("name")}
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-[#0F1F4B] mb-1.5">{f.email}</label>
                      <input
                        {...register("email")}
                        type="email"
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-[#0F1F4B] mb-1.5">{f.subject}</label>
                    <input
                      {...register("subject")}
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-[#0F1F4B] mb-1.5">{f.message}</label>
                    <textarea
                      {...register("message")}
                      rows={5}
