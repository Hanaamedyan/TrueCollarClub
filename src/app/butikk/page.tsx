    ...
    <>
      ...
      <section className="pt-32 pb-16 bg-[#F8F7F4]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h1
              className="text-5xl font-black text-[#0F1F4B] mb-3"
              style={{ fontFamily: "Montserrat, sans-serif" }}
            >
              {tr.shop.title}
            </h1>
            <p className="text-gray-500 text-lg">{tr.shop.subtitle}</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              {filterKeys.map(({ key, label }) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
                    category === key
                      ? "bg-[#0F1F4B] text-white shadow-navy"
            ...
            <div className="flex items-center gap-2 text-sm text-gray-500">
              ...
              <span className="font-medium">{tr.shop.sortBy}:</span>
              <select
                value={sort}
                onChange={(e) => setSort(e.target.value as SortKey)}
                className="border border-gray-200 rounded-lg px-3 py-2 text-sm font-medium text-[#0F1F4B] bg-white focus:outline-none focus:border-[#0F1F4B]"
              >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-24 text-gray-400">
              <p className="text-2xl font-black mb-2" style={{ fontFamily: "Montserrat, sans-serif" }}>
                {tr.shop.noProducts}
              </p>
            </div>
          ) : (
